...
class ZeroDTE_v1_2_App:
    ...
    def analizar(self, d):
        ...
        return "STAY ASIDE", {"Motivo": "Volatilidad errática o pánico"}

# INTERFAZ STREAMLIT
st.title("🎯 0DTE v1.2 Sniper & Trend")
st.sidebar.header("Configuración de Cuenta")
saldo_input = st.sidebar.number_input("Saldo Actual (€)", value=25000, step=1000)

if st.button("Ejecutar Auditoría de Mercado"):
    try:
        ...
        # Decisión
        st.subheader(f"Recomendación: {res}")
        if res != "STAY ASIDE":
            st.success(f"Ejecutar estrategia con {det['Contratos']} contratos.")
            st.info(f"Crédito Mínimo (Regla 1/3): ${det['Credito']:.2f}")
            
            # Gráfico de Perfil de Riesgo (Simulado)
            fig = go.Figure()
...
st.markdown("---")
st.caption("Recuerda ejecutar 45min después de la apertura. Cierre 50% TP o 21:30h ESP.")
